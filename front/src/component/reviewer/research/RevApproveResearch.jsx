...
export default class RevApproveResearch extends Component {
...
onApprove = (e) =>{
    ...
    const mTitle = this.state.paper.mTitle;
    const userId = this.state.paper.userId;
    ...
    axios.put(`http://localhost:8000/rev/researchpaper/update/${id}`,Approvedata).then((res) =>{
        if(res.data.success){
        }
    });

    axios.post("http://localhost:8000/rev/researchpapernotify/save" ,notifydata).then((res) =>{
        if(res.data.success){
            alert("Research Paper Approved")
        }
    });
...
onDecline = (e) =>{
    ...
    axios.put(`http://localhost:8000/rev/researchpaper/update/${id}`,data).then((res) =>{
        if(res.data.success){
        }
    });
...
componentDidMount(){
    ...
    axios.get(`http://localhost:8000/rev/researchpaper/${id}`).then((res) =>{
        if(res.data.success){
            this.setState({
    ...
    render() {
        const {mTitle,category,abstract,keyword,author,status,date,photo,userId,first_name,last_name,con_number} = this.state.paper;
                ...
                <div>
                    ...
                    >
                        ...
                        <br/>
                            <center>
                            <br/><br/><br/>
                                <h1 className="black-text font-italic font-weight-bold">{mTitle}</h1>
                            </center>
                        </div>
                    </div>
                        ...
                        <MDBRow>
                                ...
                                <MDBRow>
                                    ...
                                    <MDBTableHead>
                                        ...
                                        <th className='font-weight-bold'>
                                            <h3 className="text-center">Research Details</h3>
                                        </th>
                                        </tr>
                                    </MDBTableHead>
                                    <MDBTableBody>                      
                                    ...
                                    <MDBCardHeader className="form-header blue-grey rounded">
                                       ...
                                       Read carefully before get Action to this research paper
                                            ...
                                            <button className="btn btn-success btn-sm" type="type" style={{marginTop:"15px"}} onClick={this.onApprove}> 
                                                <Link class="nav-link white-text" to="/rev/research/reviewed"><MDBIcon icon="file-alt" />  APPROVE</Link>                 
                                            </button>
                                            <hr/>
                                            <button className="btn btn-danger btn-sm" type="type" style={{marginTop:"15px"}} onClick={this.onDecline}>
                                                <Link class="nav-link white-text" to="/rev/research/reviewed"><MDBIcon icon="file-alt" />  DECLINE</Link>                      
                                            </button>
                                    </MDBCardHeader>
                            ...
                            <MDBCol md='4'>
                                ...
                                <MDBCardBody className="winter-neva-gradient">
                                    ...
                                    <MDBIcon icon='file-download' /> DOWNLOAD RESEARCH PAPER
                                    </a>
                                </MDBCardBody>
                                </MDBCard>
                                </MDBRow>
...
export default RevApproveResearch;
